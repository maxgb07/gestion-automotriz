...
<div class="max-w-7xl mx-auto py-8">
    @php 
        ...
        // Agregar Ventas pagadas en el periodo
        foreach($ventas as $v) {
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
        ...
        <div class="flex gap-4">
            <a href="{{ route('reportes.ventas.pdf', ['fecha_inicio' => $fecha_inicio, 'fecha_fin' => $fecha_fin]) }}" target="_blank" class="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-black rounded-xl transition-all uppercase tracking-widest flex items-center justify-center cursor-pointer shadow-lg shadow-blue-900/40">
    ...
    <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 mb-8 shadow-2xl">
        <form action="{{ route('reportes.ventas') }}" method="GET" class="flex flex-col md:flex-row items-end gap-6">
            ...
            <button type="submit" class="px-10 py-3 bg-white/10 hover:bg-white/20 text-white text-xs font-black rounded-xl transition-all uppercase tracking-widest flex items-center justify-center border border-white/10 shadow-lg">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        <div class="bg-blue-600/20 backdrop-blur-xl rounded-2xl p-5 border border-blue-500/30 shadow-xl">
            <p class="text-blue-200 uppercase font-bold tracking-widest text-md mb-1">Total Ventas</p>
                ...
                <tbody class="divide-y divide-white/10">
                    @forelse($combined as $mov)
                        ...
                        <tr class="hover:bg-white/5 transition-colors">
                            <td class="px-4 py-4">
                                <span class="px-3 py-1 rounded-lg text-md font-black tracking-widest uppercase {{ $mov['es_pago'] ? 'bg-orange-500/20 text-orange-300' : 'bg-purple-500/20 text-purple-300' }}">
                                    {{ $mov['label'] }}
                                </span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-20 text-center">
                                <p class="text-blue-200/30 uppercase font-black tracking-widest italic text-base">No hay ventas registradas en este periodo</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
