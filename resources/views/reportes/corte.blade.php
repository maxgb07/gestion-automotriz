...
<div class="mx-auto py-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start mb-6 gap-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-black text-white uppercase tracking-tighter">Corte del Día</h1>
                <p class="text-white font-black text-lg tracking-widest mt-0.5">{{ $hoy->isoFormat('LL') }}</p>
            </div>
        </div>
        <a href="{{ route('reportes.corte.pdf') }}?fecha={{ $fecha }}" target="_blank"
            class="inline-flex items-center justify-center px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-black rounded-xl transition-all uppercase tracking-widest shadow-lg shadow-blue-900/30 whitespace-nowrap">
    ...
    {{-- Filtro de Fecha (idéntico al estilo de Ordenes/Ventas) --}}
    <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 mb-8 shadow-2xl">
        <form action="{{ route('reportes.corte') }}" method="GET" class="flex flex-col md:flex-row items-end gap-6">
            <div class="flex-grow">
                <label class="block text-md font-black text-blue-200 uppercase tracking-widest mb-2 ml-1">Día a Consultar</label>
